<template>
  <div class="swipe-container">
    <div
      class="profile-card"
      v-for="(profile, index) in profiles"
      :key="index"
      :style="{
        zIndex: profiles.length - index,
        transform: `translateX(${profile.offsetX}px) rotate(${profile.rotate}deg)`,
        opacity: profile.opacity
      }"
      @mousedown="startSwipe($event, profile, index)"
      @touchstart="startSwipe($event, profile, index)"
    >
      <img :src="profile.image" alt="Profile" class="profile-image" />
      <div class="profile-info">
        <h3>{{ profile.name }}, {{ profile.age }}</h3>
        <p>{{ profile.bio }}</p>
      </div>
    </div>

    <!-- Swipe Buttons (inside container) -->
    <div class="swipe-buttons">
      <button class="swipe-btn dislike-btn" @click="manualSwipe('dislike')">
        <i class="fas fa-times"></i>
      </button>
      <button class="swipe-btn like-btn" @click="manualSwipe('like')">
        <i class="fas fa-heart"></i>
      </button>
    </div>
  </div>
</template>

<script setup>
import { ref, reactive } from 'vue';

// Sample profile data with random picsum images
const profiles = ref([
  { name: 'Alice', age: 25, bio: 'Love traveling and photography.', image: 'https://picsum.photos/400/600?random=1', offsetX: 0, rotate: 0, opacity: 1 },
  { name: 'Bob', age: 30, bio: 'Avid hiker and coffee enthusiast.', image: 'https://picsum.photos/400/600?random=2', offsetX: 0, rotate: 0, opacity: 1 },
  { name: 'Clara', age: 28, bio: 'Book lover and food explorer.', image: 'https://picsum.photos/400/600?random=3', offsetX: 0, rotate: 0, opacity: 1 },
  { name: 'samuel', age: 28, bio: 'Book lover and food explorer.', image: 'https://picsum.photos/400/600?random=3', offsetX: 0, rotate: 0, opacity: 1 },
  { name: 'jackson', age: 28, bio: 'Book lover and food explorer.', image: 'https://picsum.photos/400/600?random=3', offsetX: 0, rotate: 0, opacity: 1 },
]);

const currentIndex = ref(0);
const swipeData = reactive({
  startX: 0,
  currentX: 0,
  dragging: false,
});

const startSwipe = (event, profile, index) => {
  swipeData.startX = event.type === 'mousedown' ? event.clientX : event.touches[0].clientX;
  swipeData.dragging = true;

  const moveHandler = (moveEvent) => {
    swipeData.currentX = moveEvent.type === 'mousemove' ? moveEvent.clientX : moveEvent.touches[0].clientX;
    const deltaX = swipeData.currentX - swipeData.startX;
    profiles.value[index].offsetX = deltaX;
    profiles.value[index].rotate = deltaX / 10;
    profiles.value[index].opacity = Math.max(0.5, 1 - Math.abs(deltaX) / 400);
  };

  const endSwipe = () => {
    swipeData.dragging = false;
    const deltaX = swipeData.currentX - swipeData.startX;
    if (deltaX > 150) {
      like(); // Right swipe (like)
    } else if (deltaX < -150) {
      dislike(); // Left swipe (dislike)
    } else {
      resetCard(index); // Reset card position if swipe is too small
    }
    window.removeEventListener('mousemove', moveHandler);
    window.removeEventListener('mouseup', endSwipe);
    window.removeEventListener('touchmove', moveHandler);
    window.removeEventListener('touchend', endSwipe);
  };

  window.addEventListener('mousemove', moveHandler);
  window.addEventListener('mouseup', endSwipe);
  window.addEventListener('touchmove', moveHandler);
  window.addEventListener('touchend', endSwipe);
};

const like = () => {
  animateSwipe('like');
};

const dislike = () => {
  animateSwipe('dislike');
};

const animateSwipe = (direction) => {
  const index = currentIndex.value;
  const profile = profiles.value[index];
  profile.offsetX = direction === 'like' ? 400 : -400;
  profile.rotate = direction === 'like' ? 30 : -30;
  profile.opacity = 0;
  setTimeout(() => {
    profiles.value.splice(index, 1);
  }, 300);
};

const resetCard = (index) => {
  profiles.value[index].offsetX = 0;
  profiles.value[index].rotate = 0;
  profiles.value[index].opacity = 1;
};

const manualSwipe = (direction) => {
  if (direction === 'like') {
    like();
  } else {
    dislike();
  }
};
</script>

<style scoped>
/* Container */
.swipe-container {
  position: relative;
  height: calc(100vh - 200px); /* Adjusted height based on navbar and bottom nav */
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  background-color: #f4f4f4;
  padding: 1rem;
  overflow: hidden;
}

/* Profile Card */
.profile-card {
  position: absolute;
  width: 90%;
  max-width: 400px;
  aspect-ratio: 3/4;
  background-color: #fff;
  border-radius: 10px;
  box-shadow: 0 8px 20px rgba(0, 0, 0, 0.2);
  overflow: hidden;
  transition: transform 0.5s ease, opacity 0.5s ease;
}

.profile-image {
  width: 100%;
  height: 65%;
  object-fit: cover;
}

.profile-info {
  padding: 1rem;
}

.profile-info h3 {
  margin-bottom: 0.5rem;
  font-size: 1.5rem;
}

.profile-info p {
  color: #555;
}

/* Swipe Buttons inside the container */
.swipe-buttons {
  position: absolute;
  bottom: 40px;
  display: flex;
  justify-content: space-between;
  width: 100%;
  max-width: 250px;
  z-index: 10;
}

.swipe-btn {
  width: 60px;
  height: 60px;
  border-radius: 50%;
  border: none;
  display: flex;
  justify-content: center;
  align-items: center;
  font-size: 1.5rem;
  cursor: pointer;
  transition: transform 0.3s ease, background-color 0.3s ease;
}

.swipe-btn i {
  color: white;
}

/* Dislike Button Style */
.dislike-btn {
  background-color: #dc3545;
}

.dislike-btn:hover {
  transform: scale(1.1);
  background-color: #c82333;
}

/* Like Button Style */
.like-btn {
  background-color: #28a745;
}

.like-btn:hover {
  transform: scale(1.1);
  background-color: #218838;
}

/* Responsive Design */
@media (min-width: 600px) {
  .swipe-btn {
    width: 70px;
    height: 70px;
    font-size: 1.8rem;
  }
}
</style>
